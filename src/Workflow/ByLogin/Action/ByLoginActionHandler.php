 ...
 */
class ByLoginActionHandler extends BaseActionHandler
{
    /**
     * @var Services
     */
    ...
    public function performActions(
        ...
        switch ($type) {
            case 'text':
                $logins = $object_config['content']['text_list'] ?? '';
                break;
            case 'file':
                ...
                $logins = (string) $this->irss->consume()->stream($rid)->getStream();
                break;
            default:
                $logins = '';
        }
        ...
        try {
            $person_list = $this->person_list_generators->byLoginsFromString(
                $logins
            );
